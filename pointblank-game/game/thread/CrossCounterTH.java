...
public class CrossCounterTH implements PrincipalAbstract
{
	@Override
					...
					if (value == 1)
					{
						for (int i = 0; i < 16; i++)
						{
							...
							if (pR != null && s.state == SlotState.BATTLE)
							{
								s.espectador = false;
								s.death = false;
								pR.connection.sendPacket(new BATTLE_MISSION_ROUND_PRE_START_ACK(r));
							}
						}
						r.stopTask(ThreadState.CROSSCOUNTER);
						r.threadCROSS = null;
						r.crossStart(new CrossCounterTH(), 0, 5350);
					}
					else
					{
						for (int i = 0; i < 16; i++)
