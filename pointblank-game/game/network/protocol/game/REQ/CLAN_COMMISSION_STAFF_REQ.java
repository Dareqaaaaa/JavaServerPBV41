...
public class CLAN_COMMISSION_STAFF_REQ extends game.network.protocol.GamePacketREQ
{
	protected int sucess;
	public CLAN_COMMISSION_STAFF_REQ(int opcode)
	{
		super(opcode);
	}
	@Override
	...
	public void runImpl()
	{
		sendPacket(new CLAN_COMMISSION_STAFF_ACK(sucess));
	}
	public void sendMessage(Clan c, Player p) throws Exception
	{
		if (c != null && p != null)
		{
			PlayerMessage msg = new PlayerMessage(p.id, c.id, 15, NoteType.WEB, NoteReceive.AUXILIAR_CLAN, false);
			msg.name = c.name;
